...
function firstHTML()
{
	...
	s='<div id="message"><div style="width:100%;overflow:hidden;"><div style="float:left;width:1px;margin-left:-1px;height:6.67em;"></div><div id="messdiv">DongPing DhtmlXQ ChessBoard Loading.....<br>Powered By dpxq.com hldcg Ver '+PVer+c+'</div></div><div style="margin:6px;text-align:center;"><button class="messbtn" style="margin-right:20%;" onclick="clickYES()">&nbsp;\u78BA \u5B9A&nbsp;</button><button class="messbtn" onclick="clickNO()">&nbsp;\u53D6 \u6D88&nbsp;</button></div></div>';
	d.id='DhtmlXQ';
...
function initWH(fdbs)
{
	...
	if (Mobile)
	{
		wDevice=document.documentElement.clientWidth;   // \u8A2D\u5099\u5BEC\u5EA6
		hDevice=document.documentElement.clientHeight;  // \u8A2D\u5099\u9AD8\u5EA6
...
function getInfo()
{
	...
	s+='\u68CB\u8B5C:'+(TT[5]-TT[4])+'ms\n';
	s+='\u7248\u672C:'+PVer+CVer+'\n';
	s+='\u986F\u793A:'+(Mobile?'\u624B\u6A5F':VIMG?'\u5716\u5F62':'\u6587\u672C')+'\n';
	s+='\u500D\u6578:'+FDBS+' '+(t==350?'\u7121\u6CE8\u89E3\u8B8A\u8457':(t==240?'\u53EA\u57FA\u672C\u68CB\u76E4':''))+'\n';
	s+='\u5BEC\u5EA6:'+(Mobile?wDevice:w)+'px\n';
	s+='\u9AD8\u5EA6:'+(Mobile?(parseInt($('btn_search').style.top)+parseInt($('btn_search').style.height)+'px '+hDevice):h)+'px\n';
	s+='\u6587\u5B57:'+fz+'px\n';
	s+='\u984C\u5B57:'+fzt+'px '+$('div1').style.fontSize+'\n';
	s+='\u984C\u9AD8:'+$('div1').clientHeight+'px\n';
	s+='\u68CB\u683C:'+qgdx+'px\n';
	s+='\u68CB\u5B50:'+qzdx+'px '+(imgQiZi?(qznum+'\u5716\u5F62'):baseQiZi?'dataURL':UWD?'WebDings':'\u5B8B\u9AD4')+'\n';
	s+='\u7CFB\u7D71:'+(WIN.w95?'Win95':WIN.w9x?'Win9X':WIN.w2000?'Win2000':(WIN.wxp==1)?'WinXP':WIN.wxp?'WinXP+':WIN.mobile?'Mobile':'other')+'\n';
	s+='\u8A9E\u8A00:'+(ZHCN?'':'\u975E')+'\u7C21\u9AD4\u4E2D\u6587\n';
	s+='\u700F\u89BD:'+(NS.IE?('IE '+NS.IE+Opera):NS.Firefox?('Firefox '+NS.Firefox+Opera):NS.Chrome?('Chrome '+NS.Chrome):NS.Safari?('Safari '+NS.Safari):NS.Opera?('Opera '+NS.Opera):NS.WebKit?('WebKit '+NS.WebKit):NS.Gecko?('Gecko '+NS.Gecko+Opera):'other')+'\n';
	s+='\u6A21\u5F0F:'+(CSS1?'CSS1Compat':'BackCompat')+'\n';
	s+='\u7A7A\u683C:'+(USPACE?'\u7A7A\u683C':'&nbsp;')+'\n';
	s+='\u6578\u5BEC:'+(Mobile?'1.5':'2')+'*fz'+(sMnum==''?'=':(Mobile?1.5:2)*fz<i?'<':'>')+i+'px\n';
	s+='\u6587\u4EF6:NoFile='+NoFile+'\n';
	s+='\u540C\u57DF:TongYu='+TongYu+'\n';
	s+='\u641C\u7D22:isSP='+isSP+'\n';
	s+='\u6B63\u5247:'+(REOK?'':'\u4E0D')+'\u652F\u6301.+?\n';
	...
	if (imgQiZi)
	{
		...
		{
			h=[0,1,2,3,4,9,11];
			for(i=1;i<8;i++) s+='\u7D05'+tQZ.charAt(h[i-1])+':'+QZIMG[i]+'\n';
...
function showalert(sMag,sUrl,sLine,sFunc)
{
	...
	if (!!sUrl)
	{
		...
		if ((a>0&&a<15)||(b>0&&b<10)) sMag=('\u932F\u8AA4:'+sMag+' \u884C\u865F:'+sLine+'<br><nobr>\u7DB2\u5740:'+sUrl+'</nobr><br>\u8ACB\u6309Ctrl+F5\u5237\u65B0,\u5237\u65B0\u7121\u6548\u8ACB\u5831\u932F: <nobr><a target="_blank" href="tencent://message/?uin=88081492&Site=DhtmlXQ.'+location.hostname+'&Menu=yes"><img border="0" SRC="http://wpa.qq.com/pa?p=1:88081492:16" alt="\u9EDE\u64CA\u9019\u88CF\u767C\u6D88\u606F" /></a></nobr>');
		else return false;
	}
...
function initboard(FDBS)
{
	...
	var QZ='0_8 \u8ECA 1_7 \u99AC 2_6 \u76F8 3_5 \u4ED5 4 \u5E25 9_10 \u70AE 11_12_13_14_15 \u5175 16_24 \u8ECA 17_23 \u99AC 18_22 \u8C61 19_21 \u58EB 20 \u5C07 25_26 \u70AE 27_28_29_30_31 \u5352'.split(' ');
	...
	s='';
	// \u6A19\u984C
	s+=('<div id="div1" style="position:absolute;overflow:hidden;top:'+yTitle+'px;left:'+xTitle+'px;width:'+wTitle1+'px;height:'+hTitle+'px;text-align:center;font-size:16px;line-height:'+hTitle+'px;white-space:nowrap;background-color:#9cf;">\u5C4F\u5E55\u9AD8'+screen.height+'\u5C4F\u5E55\u5BEC'+screen.width+'\u8A2D\u5099\u9AD8'+window.screen.availHeight+'\u8A2D\u5099\u5BEC'+wDevice+'</div>');
	...
	s+='<input type="button" onclick="gotonum(\'First\')" style="'+btn+'width:'+wbtn1+'px;margin:'+mBtn+'px 0 0px 0px;" value="\u958B\u5C40" />';
	s+='<input type="button" onclick="gotonum(\'Prev\')" style="'+btn+'width:'+wbtn1+'px;margin:'+mBtn+'px 0 0px '+mBtn+'px;" value="\u5F8C\u9000" />';
	s+='<input type="button" onclick="gotonum(\'Next\')" style="'+btn+'width:'+wbtn1+'px;margin:'+mBtn+'px 0 0px '+mBtn+'px;" value="\u524D\u9032" />';
	s+='<input type="button" onclick="gotonum(\'Last\')" style="'+btn+'width:'+wbtn1+'px;margin:'+mBtn+'px 0 0px '+mBtn+'px;" value="\u7D42\u5C40" />';
	s+='<input type="button" id="autoplay" style="'+btn+'width:'+wbtn1+'px;margin:'+mBtn+'px 0 0px '+mBtn+'px;" onclick="AutoPlay()" value="\u81EA\u52D5" />';
	...
	s+='<input type="button" id="turn_Board" onclick="turnBoard()" style="'+btn+'width:'+wbtn1+'px;margin:0 0 0 '+mBtn+'px;" value="\u65CB\u8F49" />';
	s+='<input type="button" id="mirror_Board" onclick="mirrorBoard()" style="'+btn+'width:'+wbtn1+'px;margin:0 0 0 '+mBtn+'px;" value="\u5C0D\u7A31" />';
	s+='<input type="button" onclick="SearchP(\'tree\')" style="'+btn+'width:'+wbtn1+'px;margin:0 0 0 '+mBtn+'px;" value="\u8B8A\u8457" />';
	s+='<input type="button" onclick="SearchP(\'next\')" style="'+btn+'width:'+wbtn1+'px;margin:0 0 0 '+mBtn+'px;" value="\u5C0D\u5C40" />';
	s+='<input type="button" id="showsource" onclick="ViewOther()" style="'+btn+'width:'+wbtn0+'px;margin:0 0 0 '+mBtn+'px;" value="\u5C0E\u51FA" />';
	s+='</div>';
	// \u8457\u6CD5
	...
	s+='<input type="button" onclick="TXTtoDhtmlXQ(cText)" style="'+btn+'width:'+(fzMove*5)+'px;margin:'+mBtn+'px 0 0 '+wMargin+'px;" value="\u6DFB\u52A0\u8457\u6CD5" />';
	s+='<input type="button" onclick="DelMove()" style="'+btn+'width:'+(wbtn4)+'px;margin:'+mBtn+'px 0 0 '+wMargin+'px;" value="\u522A" />';
	...
	s+=('<div id="move_0" onclick="gotonum(0)" style="'+smvtv+'background-color:'+bsColor+';color:#fff;" nowrap><span id="m_0_n" style="display:inline-block;">===</span><span id="m_0_m">\u68CB\u5C40\u958B\u59CB</span><span id="m_0_c"> </span><span id="m_0_v"> </span></div>');
	s+=('</div></div>');
	// \u8B8A\u8457
	...
	s+='<input type="button" onclick="copyTEXT()" style="'+btn+'width:'+wbtn2+'px;margin:'+mBtn+'px 0 0 0;font-size:12px;" value="\u8907\u88FD\u6587\u672C" />';
	s+='<input type="button" onclick="copyUBB()" style="'+btn+'width:'+wbtn2+'px;margin:'+mBtn+'px 0 0 '+wMargin+'px;font-size:12px;" value="\u8907\u88FDUBB" />';
	s+='<input type="button" onclick="copysurl(1)" style="'+btn+'width:'+wbtn2+'px;margin:'+mBtn+'px 0 0 '+wMargin+'px;font-size:12px;" value="\u8907\u88FD\u7DB2\u5740" />';
	s+='<input type="button" onclick="copyFENTEXT()" style="'+btn+'width:'+wbtn2+'px;margin:'+mBtn+'px 0 0 '+wMargin+'px;font-size:12px;" value="\u8907\u88FDFEN" />';
	s+='<input type="button" onclick="getBinit(\'xqqb\')" style="'+btn+'width:'+wbtn2+'px;margin:'+mBtn+'px 0 0 '+wMargin+'px;font-size:12px;" value="\u7C98\u8CBCFEN" />';
	s+='<input type="button" onclick="hideButton(1)" style="'+btn+'width:'+wbtn3+'px;margin:'+mBtn+'px 0 0 '+wMargin+'px;font-size:12px;" value="\u6B98\u5C40'+(wDevice>=360?'\u8A2D\u7F6E':'')+'" />';
	s+='</div>';
	// \u5C0F\u68CB\u5B50
	...
	s+='<input type="button" onclick="setBinit()" style="'+btn+'width:'+(wbtn2*3+wMargin*2)+'px;margin:'+mBtn+'px 0 0 0;font-size:12px;" value="\u7576\u524D\u5C40\u9762\u8A2D\u7232\u521D\u59CB" />';
	s+='<input type="button" onclick="setBinit(2)" style="'+btn+'width:'+wbtn2+'px;margin:'+mBtn+'px 0 0 '+wMargin+'px;font-size:12px;" value="\u6E05\u7A7A\u68CB\u76E4" />';
	s+='<input type="button" onclick="setBinit(3)" style="'+btn+'width:'+wbtn2+'px;margin:'+mBtn+'px 0 0 '+wMargin+'px;font-size:12px;" value="\u65B0\u5C40" />';
	s+='<input type="button" onclick="hideButton(2)" style="'+btn+'width:'+wbtn3+'px;margin:'+mBtn+'px 0 0 '+wMargin+'px;font-size:12px;" value="\u8907\u88FD'+(wDevice>=360?'\u6309\u9215':'')+'" />';
	s+='</div>';
	s+=('</div>');
	// \u57F7\u884C\u51FD\u6578\u9700\u8981\u7684
	...
	s+=('<div id="move_0" onmouseup="gotonum(0)" style="'+smvtv+'background-color:'+bsColor+';color:#fff;" nowrap><span id="m_0_n" style="display:inline-block;">====</span><span id="m_0_m">\u68CB\u5C40\u958B\u59CB</span><span id="m_0_c"> </span><span id="m_0_v"> </span></div>');
	s+=('</div>');
	s+=('</div>');
	...
	s+=('<input type="button" style="'+bs+'left:0;" value="'+((TongYu&&location.hostname.indexOf('dpxq')!=-1)?'\u7D05\u5305':'\u7814\u7A76')+'" id="showmini" onclick="ViewMini()">');
	...
	var ZFB=' <a href="javascript:ViewZFB(0)" style="color:#F00;">\u9818\u53D6\u652F\u4ED8\u5BF6\u7D05\u5305</a> <a href="javascript:ViewZFB(1)" style="color:#F00;">\u6253\u8CDE\u52D5\u614B\u68CB\u76E4\u4F5C\u8005</a>';
	...
	s+=cDIV(6,1,w4-7,'',0,typ,'','\u5C0E\u5165\u5C0E\u51FA'+ZFB);
	...
	s+=('<br>\u6DFB\u52A0\u68CB\u5B50:<span id="addred"><a href="javascript:addQizi(\'0_8\')" style="color:#F00;">\u8ECA</a><a href="javascript:addQizi(\'1_7\')" style="color:#F00;">\u99AC</a><a href="javascript:addQizi(\'9_10\')" style="color:#F00;">\u70AE</a><a href="javascript:addQizi(\'11_12_13_14_15\')" style="color:#F00;">\u5175</a><a href="javascript:addQizi(\'2_6\')" style="color:#F00;">\u76F8</a><a href="javascript:addQizi(\'3_5\')" style="color:#F00;">\u4ED5</a><a href="javascript:addQizi(\'4\')" style="color:#F00;">\u5E25</a></span> <a href="javascript:addQizi(\'16_24\')">\u8ECA</a><a href="javascript:addQizi(\'17_23\')">\u99AC</a><a href="javascript:addQizi(\'25_26\')">\u70AE</a><a href="javascript:addQizi(\'27_28_29_30_31\')">\u5352</a><a href="javascript:addQizi(\'18_22\')">\u8C61</a><a href="javascript:addQizi(\'19_21\')">\u58EB</a><a href="javascript:addQizi(\'20\')">\u5C07</a>');
	}
	// \u96FB\u8166\u7248\u7D50\u675F
	...
	s+=('<br>\u6CE8\u89E3\u884C\u9996<input type="text" name="zhustr" value="0"'+si+'> \u8457\u6CD5\u884C\u9996<input type="text" name="lspace" value="4"'+si+'> \u8457\u6CD5\u884C\u5C3E<input type="text" name="rspace" value="0"'+si+'>');
	s+=('<br>\u56DE\u5408\u5167\u5DE6<input type="text" name="lpad" value="1"'+si+'> \u56DE\u5408\u5167\u53F3<input type="text" name="rpad" value="1"'+si+'> \u56DE\u5408\u4E4B\u9593<input type="text" name="mspace" value="3"'+si+'>');
	s+=('<br>\u6CE8\u89E3\u6A19\u793A<input type="text" name="zhujie" value="*"'+si+'> \u8B8A\u8457\u6A19\u793A<input type="text" name="bianzhao" value="B"'+si+'> \u8B8A\u8457\u5C64\u6578<input type="text" name="maxdepth" value="999"'+si+'>');
	s+=('<br>\u6E05\u9664\u7A7A\u9805<input type="text" name="delempty" value="1"'+si+'> \u8457\u6578\u8A18\u6578<input type="text" name="istwo" value="0"'+si+'> \u6309\u5C64\u7DE8\u865F<input type="text" name="bianhao" value="1"'+si+'>');
	...
	s+=cDIV(6,1,w4-7,'',0,typ,'','\u68CB\u8B5C\u641C\u7D22'+ZFB);
...
function initdata()
{
	...
	if (PVer<getvar('pver')) xqtitle=('\u8ACB[<a href="http://www.dpxq.com/hldcg/dhtmlxq/" target="_black" style="color:#f00">\u9EDE\u6B64\u4E0B\u8F09</a>]\u6700\u65B0\u68CB\u76E4\u652F\u6301\u6587\u4EF6 <a href="javascript:getInfo()">\u7248\u672C\u4FE1\u606F</a>');
	...
	MoveDIV.innerHTML='<div id="move_0" onmouseup="gotonum(0)" style="'+smvtv+'background-color:'+bsColor+';color:#fff;" nowrap><span id="m_0_n" style="'+sMnum+'">'+(MM?'':'=')+'===</span><span id="m_0_m">\u68CB\u5C40\u958B\u59CB</span><span id="m_0_c">'+havecomment(0,0)+'</span><span id="m_0_v"> </span></div>';
...
function VarAdd(m,cm)
{
	...
	var r='<table width="100%" height="100%" cellspacing="2" cellpadding="0"><tr><td style="background-color:'+bsColor+';text-align:center;'+p+'"><a href="javascript:getMove(\''+m+'\')" style="'+sfont+l+'color:#ff0;">\u9EDE\u6B64\u589E\u52A0\u8B8A\u8457'+cm+'</a></td></tr><tr><td style="background-color:'+bsColor+';text-align:center;'+p+'"><a href="javascript:getMove(\''+m+'\',0)" style="'+sfont+l+'color:#ff0;">\u589E\u52A0\u65B0\u8B8A\u8457\u4E26\u8A2D\u7232\u4E3B\u8B8A</a></td></tr></table>';
...
function ShowCommentText()
{
	...
	if (rbe)
	{
		if (isJieQi&&rbe.slice(-2)=='\u4F4D\u932F') rbe='';
		else
		{
			$('divrbe').innerHTML='<span style="color:#f0f">\u7B2C'+nnum+'\u8457\uFF1A'+rbe+'</span>';
...
function ViewSearch()
{
	var s1='\u68CB\u8B5C\u641C\u7D22',s2='\u96B1\u85CF\u641C\u7D22',t=$('showsearch').value;
...
function copycode(obj,t)
{
	...
	else
	{
		...
		if (!cc)
		{
			showalert('<textarea rows="2" style="width:98%;margin-top:1px;word-wrap:break-word;resize:none;outline:none;left:1%;" id="CtrlC" value=""></textarea><br>\u60A8\u7684\u700F\u89BD\u5668\u4E0D\u652F\u6301\u81EA\u52D5\u8907\u88FD'+(wTitle==240?'<br>':'')+'\u8ACB\u9F20\u6A19\u53F3\u9375\u6216\u6309Ctrl+C\u8907\u88FD');
			$('CtrlC').value=s;
...
function ViewZFB(s)
{
	...
	$('showsearch').value='\u96B1\u85CF\u641C\u7D22';
	hideSource();
}
function ViewMini(s)
{
	if (LiveID>0&&!isSP) {LiveShow();return;}
	if ($('showmini').value=='\u7D05\u5305') {ViewZFB(0);return;}
	...
	else
	{
		...
		if (temp!=null)
		{
			...
			if (stype!=0&&(i<=21||(i<=100&&v==2)))
			{
				...
				if (isTwo!='') tree=tree.replace(/\u3010\u8B8A(\d+)\-(\d+)\-(\d+)\u3011/gi,'\u8B8A{$1}\u63A5\u8B8A{$2}\u7B2C$3\u8457');
...
function matchXQStudio(s)
{
	...
	s=s.replace(/\u250C/gi,'(\x01').replace(/\u2518/gi,'\x01)').replace(/[^\u8ECA\u99AC\u76F8\u4ED5\u5E25\u70AE\u5175\u8C61\u58EB\u5C07\u5352\x00-\xFF]/gi,'  ').replace(/\(\x01 /gi,'\n (\u3000)').replace(/ \x01\)/gi,'(\u3000) \n');
...
function getBinit(t)
{
	...
	if (p=='') showalert('\u8ACB'+(clip?'\u5F9E\u8EDF\u4EF6\u88CF\u8907\u88FD'+ty+'\uFF0C\u6216':'')+'\u5C07'+ty+'\u7C98\u8CBC\u5230\u4E0A\u9762\u7684\u6587\u672C\u6846\u5167');
	else if (clip) DrawPOS(p,1);
	else showalert("\u767C\u73FE"+ty+"\uFF0C\u9EDE\u78BA\u5B9A\u5C0E\u5165\u5C40\u9762|DrawPOS('"+p+"',1)");
}
function setBinit(t)
{
	if (isTurn) turnBoard();
...
function copysurl(ty)
{
	...
	if (surl!='')
	{
		...
		ty.value=surl+'\r\n'+t;
		copycode(ty,'\u5DF2\u6210\u529F\u8907\u88FD\u68CB\u8B5C\u7DB2\u5740\u5230\u526A\u8CBC\u677F\uFF0C\u53EF\u9EDE\u64CA\u4EE5\u4E0B\u7DB2\u5740\u67E5\u770B'+(wTitle==500?'\u6548\u679C':'')+(wTitle==240?'\uFF0C':'<br>\u975EIE\u6838\u5FC3\u700F\u89BD\u5668')+'\u5982\u6C92\u6709'+(wTitle==500?'\u8907\u88FD':'')+'\u6210\u529F\u8ACB\u81EA\u5DF1\u7528\u9F20\u6A19\u6216\u9375\u76E4\u8907\u88FD<br><a href="'+(murl?(SURL+murl.substr(33)):surl)+'" target="_blank" style="color:#00f">'+(murl?murl:surl)+'</a>');
		ty.scrollTop=0;
	}
}
function digg(price)
...
function getMovelistString(m0,p0,nNUM,rb,smi)
{
	...
	for (i=0;i<ml;i++)
	{
		mt=m[i].match(/\d\d/gi);
		t=p90[mt[0]-0]-0;
		if ((t>=0)&&(t<=15)) ms+=c_move(getMoveText(m[i],p90,0));//0\u7D05
		...
		if (nNUM)
		{
			...
			if (rbt.length==1&&t>=0&&t<=31) rbt+=(POK[t][mt[1]-0]=='0')?(t<16?'\u7D05':'\u9ED1')+tQZ.charAt(t)+'\u4F4D\u932F':'';
			if (rbt.length>1&&smi) rbt+=' \u8B8A['+smi+'] \u7B2C'+(nNUM+i)+'\u8457';
			rbe+='\n'+rbt;
		}
	}
	if (nNUM) RBT=rbe;
...
function ENtoCH(s)
{
	s=s.replace(/(\xB3\xB5|\xDC\x87|\xA8\xAE)/gi,'\u8ECA').replace(/(\xC2\xED|\xF1\x52|\xB0\xA8)/gi,'\u99AC').replace(/(\xCF\xE0|\xAC\xDB)/gi,'\u76F8').replace(/(\xCA\xCB|\xA5\x4B)/gi,'\u4ED5').replace(/(\xCB\xA7|\x8E\x9B|\xAB\xD3)/gi,'\u5E25').replace(/(\xC5\xDA|\xAC\xB6)/gi,'\u70AE').replace(/(\xB1\xF8|\xA7\x4C)/gi,'\u5175').replace(/(\xCF\xF3|\xB6\x48)/gi,'\u8C61').replace(/(\xCA\xBF|\xA4\x68)/gi,'\u58EB').replace(/(\xBD\xAB|\x8C\xA2|\xB1\x4E)/gi,'\u5C07').replace(/(\xD7\xE4|\xA8\xF2)/gi,'\u5352');
	...
	s=s.replace(/(\xC7\xB0|\xAB\x65)/gi,'\u524D').replace(/(\xD6\xD0|\xA4\xA4)/gi,'\u4E2D').replace(/(\xBA\xF3|\xAB\xE1|\xA6\x5A|\xE1\xE1)/gi,'\u5F8C').replace(/(\xBD\xF8|\xB6\x69|\xDF\x4D)/gi,'\u9032').replace(/(\xCD\xCB|\xB0\x68)/gi,'\u9000').replace(/(\xC6\xBD|\xA5\xAD)/gi,'\u5E73');
...
function GBtoTW(s)
{
	return s.replace(/[\u8F66\u4F21\u7817\u4FE5\u7868]/gi,'\u8ECA').replace(/[\u9A6C\u508C]/gi,'\u99AC').replace(/\u5E05/gi,'\u5E25').replace(/\u5C06/gi,'\u5C07').replace(/\u540E/gi,'\u5F8C').replace(/\u8FDB/gi,'\u9032').replace(/[\u5305\u7832]/gi,'\u70AE');
}
...
function get_MS(str,rb){
	...
	if('\u524D\u4E2D\u5F8C'.indexOf(str.charAt(0))>-1&&'\u5175\u5352'.indexOf(str.charAt(1))>-1&&s19.indexOf(str.charAt(2))>-1) str=str.substr(0,1)+str.substr(2);
	...
	if('\u524D\u4E2D\u5F8C\u4E00\u4E8C\u4E09\u56DB\u4E94'.indexOf(str.charAt(0))>-1&&'\u5175\u5352'.indexOf(str.charAt(1))>-1)
	{
		...
		else if (j.length>=2)
		{
			if (j.length==2&&'\u4E00\u4E8C\u4E09\u56DB\u4E94'.indexOf(str.charAt(0))>-1)
			{
				...
				if (l>4) return '\u5175\u5352\u6578\u4E0D\u61C9\u8D85\u904E5\u500B';
				if (l==4)
				{
					str=((t=='\u4E00'||t=='\u56DB')?'\u524D':(t=='\u4E8C'?'\u4E2D':'\u5F8C'))+(t=='\u56DB'||t=='\u4E94'?col[1]:col[0])+str.substr(2);
				}
				else
				{
					if (t=='\u4E00'||t=='\u4E8C') str=(t=='\u4E00'?'\u524D':'\u5F8C')+col[0]+str.substr(2);
					else if (t=='\u4E09') str='\u524D'+col[1]+str.substr(2);
					else
					{
						l=BPos[j[1]].join('').split(man).length;
						if (l>4) return '\u5175\u5352\u6578\u4E0D\u61C9\u8D85\u904E5\u500B';
						if (l==4) str=((t=='\u4E09')?'\u524D':(t=='\u56DB'?'\u4E2D':'\u5F8C'))+col[1]+str.substr(2);
						else if (t=='\u56DB') str='\u5F8C'+col[1]+str.substr(2);
						else return '\u6B64\u6B65\u68CB['+str+']\u4E0D\u7B26\u5408\u8A18\u8B5C\u898F\u5247';
					}
				}
				j='duobing=2lie';
				col='';
			}
			else return ('\u542B\u6709'+((rb==1)?'\u9ED1\u5352':'\u7D05\u5175')+'\u81F3\u5C112\u500B\u7684\u7E31\u5217\u6709'+j.length+'\u5217');
		}
		else
		{
			col=j;
	...
	if ((row+'a')!='a') ;
	else if('\u524D\u4E2D\u5F8C'.indexOf(str.charAt(0))>-1&&s19.indexOf(str.charAt(1))>-1)
	{
		j='';
		man=(rb==1)?'p':'P';
	...
	else if('\u8ECA\u99AC\u70AE\u5175\u5352\u76F8\u8C61\u4ED5\u58EB\u5E25\u5C07'.indexOf(str.charAt(0))>-1&&s19.indexOf(str.charAt(1))>-1)
	{
...
function chkP64(p64,rb)
{
	...
	c=tQZ.split('');
	b='_'+p64.join('_');
	if (b.search(/(_[0-8]\d).*\1/)!=-1) return '\u68CB\u5B50\u91CD\u758A';
	if (b.search(/_9[0-8]/)!=-1) return '\u68CB\u5B50\u8D8A\u754C';
	for (i=0;i<32;i++) {if (POK[i][p64[i]-0]=='0') {if (rb==0) isJieQi=1;return (i>15?'\u9ED1':'\u7D05')+c[i]+'\u4F4D\u932F';}}
...
function initPOK()
{
	...
	var z=[];
	z[0]=(s+s+s+s+s+s+s+s+s+t).split(''); //\u8ECA\u99AC\u70AE
	z[1]=z[0];
	z[2]=(f+r+g+r+f+r+g+r+f+t).split(''); //\u76F8
	z[3]=(r+r+r+j+k+j+r+r+r+t).split(''); //\u4ED5
	z[4]=(r+r+r+o+o+o+r+r+r+r).split(''); //\u5E25
	z[5]=z[3];
	z[6]=z[2];
	z[7]=z[0];
	z[8]=z[0];
...
function chkMove(m,p90,rb)
{
	...
	rbt=rb=='b'?'r':rb=='r'?'b':'u';
	if (m0==9||m2==9) return rbt+'\u8DD1\u51FA\u68CB\u76E4';
	mc=p90[m01-0]-0; //\u9EDE\u5B50id
	if (mc==99) return rbt+'\u627E\u4E0D\u5230\u5B50';
	if (m01==m23) return rbt+'\u68CB\u5B50\u672A\u52D5';
	rbt=n[mc]=='b'?'b\u9ED1':'r\u7D05';
	if (rb==n[mc]) return rbt+'\u65B9\u9023\u8D70';
	var fp=e[mc];
	if (fp=='C')
	{
	...
	if (ok.indexOf('_'+m23)==-1) return rbt+c[mc]+'\u8D70\u932F';
	// if (POK[mc][m23-0]=='0') return rbt+c[mc]+'\u4F4D\u932F';
	return n[mc];
}
...
function showRBE(nNUM)
{
	...
	if (f.length>1)
	{
		if (isJieQi&&f[1].slice(-2)=='\u4F4D\u932F') return;
		f[2]=nNUM+f[0].length;
		f[3]='\u7B2C'+f[2]+'\u8457\uFF1A'+f[1];
